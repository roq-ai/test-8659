...
import { getCompliances } from 'apiSdk/compliances';
import { ComplianceInterface } from 'interfaces/compliance';
import { getPolicies } from 'apiSdk/policies';
import { PolicyInterface } from 'interfaces/policy';
import { getFrauds } from 'apiSdk/frauds';
...
function DashboardPage() {
  ...
  const complianceStat = useSWR<PaginatedInterface<ComplianceInterface>>(
    () => `/compliances`,
    () => getCompliances({ limit: 1, offset: 0 }),
  );

  const policyStat = useSWR<PaginatedInterface<PolicyInterface>>(
    () => `/policies`,
    () => getPolicies({ limit: 1, offset: 0 }),
  );

  const fraudStat = useSWR<PaginatedInterface<FraudInterface>>(
    () => `/frauds`,
    () => getFrauds({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Compliances',
      result: complianceStat?.data?.totalCount,
      isLoading: complianceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Policies',
      result: policyStat?.data?.totalCount,
      isLoading: policyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Frauds',
      result: fraudStat?.data?.totalCount,
      isLoading: fraudStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
